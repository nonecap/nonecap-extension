...
screenshot crop, which is unchanged.
...
- **hCaptcha challenge frame** becomes a sensor + cursor host: it reports
  challenge readiness (unchanged), answers a geometry query (tile / verify /
  refresh centres in iframe-local coords, with the verify button's Skip state),
  and animates the cosmetic cursor on request. It no longer dispatches real
  pointer events.
- **page / top frame** is unchanged (pill + token detection + rect provider).
- **checkbox / anchor frame** is unchanged: the "I am human" click stays a
  synthetic in-frame click. It passes today, and if a future hCaptcha change
  ever gates it on trusted input the only consequence is the challenge opens,
  which the trusted path then solves. Deliberate simplification; revisit if
  passive-pass rates drop.

## The Skip guard (already shipped, commit aa10a23)

hCaptcha reuses `.button-submit` for both Verify/Next and Skip — it reads "Skip"
until an answer is placed. The executor now refuses to click it while it reads
Skip (`findSubmitUnlessSkip`), mirroring the internal solver's "Never Skip"
rule. This stays relevant: after a trusted drag, the button flips to Verify and
we click it; if a drag still fails for any reason, we leave it unclicked and the
round re-arms / the watchdog ends the attempt cleanly instead of skip-spamming.
...
- **The debugger banner.** While attached, Chrome shows a "NoneCap started
  ...
  present.
- **One debugger client per tab.** If DevTools is open on the tab, attach fails;
  we surface that as a normal solve failure (and could fall back to synthetic
  for grids). It also means the Playwright e2e harness (which itself holds the
  CDP session) CANNOT exercise the trusted-input path — that path is verified by
  unit tests on the driver + coordinate math plus manual testing on a real
  hCaptcha page.
